...
export default async function HistoricoGeralPage({
  ...
  const requests = await prisma.purchaseRequest.findMany({
    where: { requesterId: user?.id, ...whereCondition },
      ...
      <div className="card" style={{ marginBottom: '1.5rem' }}>
        ...
        <form method="GET" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '1rem', alignItems: 'end' }}>
          ...
          <div style={{ display: 'flex', gap: '0.5rem' }}>
            ...
            <Link href="/dashboard/historico" className="btn" style={{ backgroundColor: '#e2e8f0', color: '#1e293b' }}>Limpar</Link>
          </div>
        </form>
      </div>
              ...
              requests.map(req => {
                ...
                return (
                  <tr key={req.id} style={{ borderBottom: '1px solid var(--border)' }}>
                    ...
                    <td style={{ padding: '1rem 0.5rem' }}>
                      <Link href={user?.role === 'ADMIN' ? `/dashboard/solicitante/pedido/${req.id}` : `/dashboard/${user?.role?.toLowerCase()}/pedido/${req.id}`} className="btn btn-primary" style={{ fontSize: '0.75rem', padding: '0.25rem 0.75rem' }}>
                        Detalhes
                      </Link>
                    </td>
                  </tr>
