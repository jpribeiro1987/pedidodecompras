...
export default async function EditarPedidoAutorizadorPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (request.currentStatus === 'ENTREGUE' || request.currentStatus === 'CANCELADA') {
    return (
      <div style={{ padding: '2rem' }}>
        ...
        <Link href={`/dashboard/autorizador/pedido/${id}`} className="btn" style={{ marginTop: '1rem' }}>Voltar</Link>
      </div>
    )
  }
  ...
  return (
    <div style={{ padding: '2rem' }}>
      <div style={{ display: 'flex', alignItems: 'center', marginBottom: '2rem', gap: '1rem' }}>
        <Link href={`/dashboard/autorizador/pedido/${id}`} className="btn" style={{ backgroundColor: '#e2e8f0' }}>Voltar</Link>
